...
def combine_loss(teacher_features, student_features, eps=1e-6):
    ...
    return t_student_loss + wasserstein_loss
...
class UCLFWPKD(nn.Module):
    ...
    def forward(self, enc_stu_fea_list, enc_tea_fea_list, dec_stu_fea_list, dec_tea_fea_list):
        ...
        for fs, ft in zip(enc_stu_res_list, enc_tea_fea_list):
            ft = ft.detach()
            b, c, t, f = fs.size()
            fs_fea = fs.permute(0, 2, 3, 1).contiguous().view(b, t, f*c)
            ft_fea = ft.permute(0, 2, 3, 1).contiguous().view(b, t, f*c)
            comb_loss = combine_loss(ft_fea, fs_fea)
            # print("---dec comb_loss time:", comb_loss)
            pred_res_enc_loss += comb_loss 
            
            fs_fea = fs_fea.view(b, t, f, c).permute(0, 2, 1, 3).contiguous().view(b, f, t*c)
            ft_fea = ft_fea.view(b, t, f, c).permute(0, 2, 1, 3).contiguous().view(b, f, t*c)
            comb_loss = combine_loss(ft_fea, fs_fea)
