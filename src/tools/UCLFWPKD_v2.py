...
def wasserstein_distance_withFW(teacher_features, student_features):
    ...
    loss = sinkhorn(teacher_features, student_features) * tea_FW_mask
    loss = loss.mean() * 0.001
    return loss


def combine_loss(teacher_features, student_features, eps=1e-6):
    cosine_loss = probabilistic_loss_perframe_withFW(teacher_features, student_features)
    t_student_loss = t_student_kernel_loss_perframe(teacher_features, student_features)
    wasserstein_loss = wasserstein_distance_withFW(teacher_features, student_features)

    return cosine_loss + t_student_loss + wasserstein_loss
...
class UCLFWPKD(nn.Module):
    ...
    def forward(self, enc_stu_fea_list, enc_tea_fea_list, dec_stu_fea_list, dec_tea_fea_list):
        ...
        for fs, ft in zip(enc_stu_res_list, enc_tea_fea_list):
            ft = ft.detach()
            b, c, t, f = fs.size()
            fs_fea = fs.permute(2, 0, 3, 1).contiguous().view(t, b, f*c)
            ft_fea = ft.permute(2, 0, 3, 1).contiguous().view(t, b, f*c)
            comb_loss = combine_loss(ft_fea, fs_fea)
            # print("---enc comb_loss time:", comb_loss)
            pred_res_enc_loss += comb_loss 

            fs_fea = fs_fea.view(t, b, f, c).permute(2, 1, 0, 3).contiguous().view(f, b, t*c)
            ft_fea = ft_fea.view(t, b, f, c).permute(2, 1, 0, 3).contiguous().view(f, b, t*c)
            comb_loss = combine_loss(ft_fea, fs_fea)
            # print("---enc comb_loss freq:", comb_loss)
            pred_res_enc_loss += comb_loss 
        ...
        for fs, ft in zip(dec_stu_res_list, dec_tea_fea_list):
            ...
            ft_fea = ft_fea.view(t, b, f, c).permute(2, 1, 0, 3).contiguous().view(f, b, t*c)
            comb_loss = combine_loss(ft_fea, fs_fea)
            # print("---dec comb_loss freq:", comb_loss)
            pred_res_dec_loss += comb_loss 
