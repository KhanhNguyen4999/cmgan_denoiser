...
def combine_loss(teacher_features, student_features, eps=1e-6):
    ...
    fs_fea = student_features.permute(2, 0, 3, 1).contiguous().view(t, b, f*c)
    ft_fea = teacher_features.permute(2, 0, 3, 1).contiguous().view(t, b, f*c)
    cosine_loss = probabilistic_loss_perframe_withFW(ft_fea, fs_fea)

    ft_fea = ft_fea.view(t, b, f, c).permute(1, 2, 0, 3).contiguous().view(b, f, t*c)
    fs_fea = fs_fea.view(t, b, f, c).permute(1, 2, 0, 3).contiguous().view(b, f, t*c)
    t_student_loss = t_student_kernel_loss_perframe(ft_fea, fs_fea)

    ft_fea = ft_fea.view(b, f, t, c).permute(1, 2, 0, 3).contiguous().view(b, t, f*c)
    fs_fea = fs_fea.view(b, f, t, c).permute(1, 2, 0, 3).contiguous().view(b, t, f*c)
    wasserstein_loss = wasserstein_distance_withFW(ft_fea, fs_fea)
